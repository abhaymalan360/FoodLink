  ...
  return (
    <section className="py-20 bg-surface-container-lowest">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12">
          <h2 className="text-3xl font-extrabold text-on-surface mb-2">Where food is needed most right now</h2>
          <p className="text-on-surface-variant font-medium text-lg max-w-2xl">Real-time alerts from NGOs and community centers. Help us route surplus food to these hotspots.</p>
        </div>

        <div className="flex flex-col lg:flex-row gap-8">
          {/* Map Preview (Left) */}
          <div className="flex-1 relative rounded-3xl overflow-hidden bg-surface-container border border-outline-variant min-h-[400px] flex flex-col group">
          ...
          <div className="w-full lg:w-[400px] flex flex-col gap-4">
            <h3 className="text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-2">Live Alerts</h3>
            
            {URGENT_REQUESTS.map((req) => (
              <div key={req.id} className="bg-surface-container-lowest border border-outline-variant p-5 rounded-2xl shadow-sm hover:shadow-md hover:border-outline transition-all cursor-pointer group">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${req.color}`} />
                    <span className="text-xs font-bold text-on-surface-variant uppercase tracking-wide">{req.urgency}</span>
                  </div>
                  <span className="text-xs font-medium text-on-surface-variant">{req.time}</span>
                </div>
                
                <h4 className="font-bold text-on-surface text-lg mb-1 group-hover:text-teal-600 transition-colors">{req.area}</h4>
